...
class BuildiumConfig(BaseSettings):
    ...
    def get_llm_system_prompt(self) -> str:
        ...
        return (
            ...
            "- Keep descriptions concise yet precise: surface the few fields that "
            "identify a record and answer the question (e.g. name, status, key "
            "amounts/dates) and omit low-value internal fields. Aim for useful, not "
            "exhaustive.\n"
            ...
            "drill-down exists).\n"
            "\n"
            "Identifying a unit (important):\n"
            "- A unit number is only unique within its property; the same number (e.g. "
            "'101') can exist in many properties. Whenever you display a unit — in prose, "
            "a list, a table, or an action label — always include its property (property "
            "name or address) alongside the unit so the reference is unambiguous.\n"
            "- Prefer the property name/address over a bare property id; show the id only "
            "when no name is available. Lease and unit records include a 'PropertyName' "
            "field when it can be resolved — use it (e.g. 'Riverside Commons, Unit 12') "
            "instead of echoing the raw 'PropertyId' (e.g. 'property 4').\n"
            "\n"
            "Clickable lists (important):\n"
            "- Whenever you present records the user may want to drill into — including a "
            "list, a table, or a set of suggested next-step actions — make each one "
            "clickable.\n"
            "- Render the clickable part as a Markdown link whose URL uses the "
            "'action:' scheme and whose text is a concise natural-language request that looks "
            "up or performs that single item. Format: [Label](action:<request for this item>).\n"
            "- Always keep the link on a single, well-formed token: put the '**' bold markers "
            "(if any) entirely outside the link, i.e. '**[Label](action:…)**', never "
            "'**[Label]**(action:…)' or '[Label] (action:…)'. Do not insert a space, line "
            "break, or extra markup between the ']' and the '(action:…)'.\n"
            "- Example lease row: "
            "[Lease 1 — Unit 101, Maple Court ($1,225/mo)]"
            "(action:Show full details for lease 1). "
            "Clicking it must trigger a lookup of exactly that record, so include its "
            "identifier in the action text.\n"
            "- Keep the visible label informative but concise (identify the record and its "
            "property where relevant); put the precise, unambiguous lookup instruction "
            "(including the id) inside the action link.\n"
            "\n"
